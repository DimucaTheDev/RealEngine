...
namespace RE.Core.World.Physics
{
    ...
    internal class PhysicObject : Renderable
    {
        ...
        public event Action<PhysicObject> ColliderEnter;
        public event Action<PhysicObject> ColliderStay;
        public event Action<PhysicObject> ColliderExit;

        public PhysicObject(ModelRenderer model, RigidBody rigidBody, bool notRenderModel = false)
        {
            Model = model;
            RigidBody = rigidBody;
        ...
        public virtual void OnColliderEnter(PhysicObject obj) { ColliderEnter?.Invoke(obj); }
        public virtual void OnColliderStay(PhysicObject obj) { ColliderStay?.Invoke(obj); }
        public virtual void OnColliderExit(PhysicObject obj) { ColliderExit?.Invoke(obj); }
        ...
        public override void Render(FrameEventArgs args)
        {
            if (RigidBody != null)
            {
                ...
                Model.Position = new Vector3(bulletTransform.Origin.X, bulletTransform.Origin.Y,
                    bulletTransform.Origin.Z);
                BulletSharp.Math.Quaternion bulletRotation =
                    BulletSharp.Math.Quaternion.RotationMatrix(bulletTransform.Basis);
        ...
        void DrawRigidBodyBounds(RigidBody body, LineManager lineManager)
        {
            ...
            if (body.CollisionShape is BoxShape boxShape)
            {
                ...
                Matrix worldTransform = body.WorldTransform;
